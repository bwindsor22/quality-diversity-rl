...
"""  # here starts the code: (beating the code folding glitch)
    ...
    def eval_vector(self, in_, opts, N, default_value=1.0):
    ...
    def __init__(self, x0, sigma0, inopts=None):
        ...
        self.cmame_feature_map = FeatureMap()
        ...
        opts['minstd'] = self.eval_vector(opts['minstd'], opts, N, 0)
        opts['maxstd'] = self.eval_vector(opts['maxstd'], opts, N, np.inf)
        ...
        if 1 < 3:  # new version with class
            self.sigma_vec0 = self.eval_vector(self.opts['CMA_stds'], opts, N)
            ...
            self.sigma_vec = self.eval_vector(self.opts['CMA_stds'], opts, N)
            if np.isfinite(self.opts['CMA_dampsvec_fac']):
                self.sigma_vec *= np.ones(N)  # make sure to get a vector
            self.sigma_vec0 = self.sigma_vec if np.isscalar(self.sigma_vec) \
                                            else self.sigma_vec.copy()
        stds = self.eval_vector(self.opts['CMA_teststds'], opts, N)
    ...
    # ____________________________________________________________
    def tell(self, individuals, check_points=None,
             copy=False):
        ...
        """
        logging.info('Fast CMAME tell, {} individuals...'.format(len(individuals)))

        self.cmame_feature_map.add_individuals_to_map(individuals)
        solutions = [ind.flattened_state_dict for ind in individuals]
    ...
    "timer" attribute not found, probably because `ask` was never called.
     ...
     """,
        ...
        if self.cmame_needs_reset(function_values):
            self.cmame_restart()

    # end tell()


    def cmame_needs_reset(self, function_values):
        min_fit = min(function_values)
        ...
        eigs = self.D ** 2
        area = self.sigma * (abs(max(eigs)) ** 0.5)
    ...
    def cmame_restart(self):
        logging.info('Restart CMAME')
        if self.cmame_feature_map.count_elites() > 0:
            individual = self.cmame_feature_map.get_random_elite()
            self.mean = array(individual.flattened_state_dict)
        else:
            self.mean = array([self.randn(0, self.sigma_start) for _ in range(self.N)])

        self.C = self.sm.covariance_matrix # covariance matrix
        self.dC = self.sm.variances
        self.D = self.dC**0.5  # we assume that the initial C is diagonal
        ...
        self.pc_neg = np.zeros(self.N)

        ## remake sampler
        opts = CMAOptions({}).complement()  # CMAOptions() == fmin([],[]) == defaultOptions()
        stds = self.eval_vector(self.opts['CMA_teststds'], opts, self.N)
        if self.opts['CMA_sampler'] is None:
            self.sm = sampler.GaussFullSampler(stds * np.ones(self.N),
                lazy_update_gap=(
                    1. / (self.sp.c1 + self.sp.cmu + 1e-23) / self.N / 10
                    if self.opts['updatecovwait'] is None
                    else self.opts['updatecovwait']),
            ...
            p = self.sm.parameters(mueff=self.sp.weights.mueff,
                                   lam=self.sp.weights.lambda_)
            self.sp.weights.finalize_negative_weights(self.N, p['c1'], p['cmu'])
