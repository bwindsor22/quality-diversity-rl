...
def fitness_feature_fn(score_strategy, stop_after, game, run_name, policy_net, env_maker,objectives):
    """
        ...
        if success:
            try:
                fitness, feature = fitness_feature_fn(task.score_strategy, task.stop_after, task.game,
                                                                self.run_name, self.model, self.env_maker,task.objectives)
                result = Result(task.run_name, task.model, feature, fitness, task.objectives)
                return result
            except Exception as e:
